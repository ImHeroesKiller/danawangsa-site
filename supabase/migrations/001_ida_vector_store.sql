-- IDA RAG: pgvector extension + document chunks table
...
create table if not exists ida_document_chunks (
  ...
  source_type text not null check (source_type in ('service', 'faq', 'legal')),
...
-- Session memory for multi-turn context (optional persistence)
