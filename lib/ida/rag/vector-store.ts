...
import type { IdaDocumentChunk, IdaRetrievedChunk } from "./types";
...
export async function clearIdaChunks(): Promise<void> {
...
export async function upsertIdaChunks(chunks: IdaDocumentChunk[]): Promise<number> {
  const supabase = getSupabaseAdmin();
  const batchSize = 50;
  let inserted = 0;
...
export async function searchIdaChunks(options: {
  query: string;
  locale: Locale;
  matchCount?: number;
  matchThreshold?: number;
}): Promise<IdaRetrievedChunk[]> {
  const {
    query,
    locale,
    matchCount = 6,
  ...
  return (data ?? []).map(
    (row: {
      ...
      section: row.section,
      sourceType: row.source_type as IdaRetrievedChunk["sourceType"],
