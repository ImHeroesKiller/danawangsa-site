...
export function formatSseEvent(
  ...
  data: IdaSseMetaPayload | IdaSseTokenPayload | IdaSseDonePayload | IdaSseErrorPayload,
): string {
  return `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
}

export function createSseStream(
  handler: (send: (event: IdaSseEventType, data: unknown) => void) => Promise<void>,
  ...
  return new ReadableStream({
    async start(controller) {
      const send = (event: IdaSseEventType, data: unknown) => {
        controller.enqueue(encoder.encode(formatSseEvent(event, data as never)));
      };
      ...
      try {
        ...
        controller.enqueue(
          encoder.encode(
            formatSseEvent("error", { error: message } satisfies IdaSseErrorPayload),
          ),
        );
      } finally {
        controller.close();
